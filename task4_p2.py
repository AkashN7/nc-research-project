...
d5 = {'LinkName': [], 'Year': [], 'Day-of-the-week': [], 'Time-of-the-day': [], 'PT': [], 'PTI': [],
      'BT': [], 'BTI': [], 'TTI': [], 'L_Skew': [], 'L_Var': [], 'AvgTT': []}

for index, row in xl.iterrows():
    name = str(row['LinkName']) + '#' + str(row['Year']) + '#' + str(row['Day-of-the-week']) +\
           '#' + str(row['Time-of-the-day'])
    if str(row['Date']) == "03/12/2017" or str(row['Date']) == "01/17/2018" or str(row['Date']) == "04/02/2019" or \
            str(row['Date']) == "09/16/2018" or str(row['Date']) == "09/17/2018":
        if name in d6:
            d6[name].append(row['Travel time'])
        else:
            d6[name] = [row['Travel time']]
    else:
        if name in d2:
